...
class EvolutionRunner:
    def __init__(
        ...
        # Pass through embedding_model as-is; None disables embeddings.
        ...
        self.scheduler = JobScheduler(
            ...
            verbose=verbose,
            max_workers=evo_config.max_parallel_jobs,
        )
        ...
        if current_gen == 0:
            ...
            for nov_attempt in range(self.evo_config.max_novelty_attempts):
                ...
                for resample in range(self.evo_config.max_patch_resamples):
                    # Debug: log DB state before sampling to aid diagnosis
                    try:
                        prog_count = self.db._count_programs_in_db()
                    except Exception:
                        prog_count = 'unknown'
                    logger.debug(
                        f"Sampling from DB (path={self.db.config.db_path}) programs_count={prog_count}"
                    )
    ...
    def _update_best_solution(self):
        ...
        if best_dir.exists():
            def _onerror(func, path, exc_info):
                # Make read-only files writable and retry removal (Windows/OneDrive).
                try:
                ...
                try:
                    ...
                    # As a last resort, ignore errors to avoid halting evolution.
                    pass

            try:
                shutil.rmtree(best_dir, onerror=_onerror)
            except PermissionError:
                # Fallback: attempt best-effort cleanup without raising.
                try:

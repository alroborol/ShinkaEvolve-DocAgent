...
class LLMClient:
    ...
    def batch_query(
        ...
        target_models = [
            ...
            for kwargs in llm_kwargs
            if isinstance(kwargs, dict) and kwargs.get("model_name") is not None
        ]
        if len(target_models) == 0:
            target_models = self.model_names
        force_single_process = any(is_ollama_model_name(name) for name in target_models)

        num_processes = 1 if force_single_process else min(num_samples, mp.cpu_count())
    ...
    def batch_kwargs_query(
        ...
        # multiprocess sample_kwargs_query
        force_single_process = any(is_ollama_model_name(name) for name in self.model_names)
        num_processes = 1 if force_single_process else min(num_samples, mp.cpu_count())
        with mp.Pool(processes=num_processes) as pool:
            # Submit all tasks asynchronously first
            async_results = []
